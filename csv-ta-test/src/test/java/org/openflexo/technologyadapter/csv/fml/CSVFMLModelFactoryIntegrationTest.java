...
public class CSVFMLModelFactoryIntegrationTest extends AbstractModelFactoryIntegrationTestCase {

	private static final Logger logger = FlexoLogger.getLogger(CSVFMLModelFactoryIntegrationTest.class.getPackage().getName());
	...
	public void initializeServiceManager() {
		...
		assertNotNull("CSV Technology Adapter should be activated", csvTA);

		log("CSV Technology Adapter successfully activated");
		log("  TA Name: " + csvTA.getName());
		log("  TA Identifier: " + csvTA.getIdentifier());
	}

	
	@Test
	@TestOrder(2)
	public void checkCSVTechnologyAdapter() {
		log("checkCSVTechnologyAdapter()");

		CSVTechnologyAdapter csvTA = serviceManager.getTechnologyAdapterService()
				.getTechnologyAdapter(CSVTechnologyAdapter.class);

		assertNotNull("CSV Technology Adapter should be available", csvTA);

		
		testVirtualModelModelFactoryWithTechnologyAdapter(csvTA);

		log("FML model factory successfully tested with CSV TA");
	}
